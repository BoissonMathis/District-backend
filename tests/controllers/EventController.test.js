...
describe("addManyEvents", () => {
  ...
  it("Evenement à ajouter, invalide. - E", (done) => {
...
describe("findOneEvent", () => {
  it("Chercher un evenement par les champs selectionnées. - S", (done) => {
    EventService.findOneEvent(
      ...
      function (err, value) {
        expect(value).to.haveOwnProperty("user");
        expect(JSON.stringify(value["user"])).to.equal(
          JSON.stringify(events[0].user)
        );
        done();
      }
    );
  ...
  it("Chercher un evenement par un champ non autorisé. - E", (done) => {
    EventService.findOneEvent(
      ["test"],
      events[0].contentText,
      null,
      function (err, value) {
        expect(err).to.be.a("object");
        expect(err).to.haveOwnProperty("msg");
        expect(err["msg"]).to.equal(
          "Les champs (test) ne sont pas des champs de recherche autorisé."
        );
        expect(err).to.haveOwnProperty("type_error");
        expect(err["type_error"]).to.equal("no-valid");
  ...
  it("Chercher un evenement avec une valeur de recherche vide. - E", (done) => {
    EventService.findOneEvent(["user"], "", null, function (err, value) {
      expect(err).to.be.a("object");
      expect(err).to.haveOwnProperty("msg");
...
describe("findManyEvents", () => {
  ...
  it("Faire une recherche sur le contentText avec une query qui ne correspond a aucun evenement - S", (done) => {
    EventService.findManyEvents(
      "awtbrieoqpmfnhgsekl",
      "contentText",
      3,
