...
module.exports.addOnePost = async function (post, options, picture, callback) {
  let opts = {
    populate:
  ...
  try {
    ...
    if (errors) {
      ...
      await new_post.save();
      console.log("Nouveau post créé :", new_post);

      if (picture) {
        console.log("Appel à updatePostImage avec l'image :", picture);
        await ImageService.updatePostImage(new_post._id, picture);
        console.log("updatePostImage a été appelé");
      }

      Post.findById(new_post._id, null, opts)
        .then((value) => {
          if (value) {
            console.log("Post trouvé après création : ", value);
    ...
    if (error.code === 11000) {
      ...
      var err = {
        msg: "Format de requête invalide",
        type_error: "validator",
      };
      callback(err);
    }
...
module.exports.findOnePostById = function (post_id, options, callback) {
  ...
  if (post_id && mongoose.isValidObjectId(post_id)) {
    Post.findById(post_id, null, opts)
      .then((value) => {
        try {
          if (value) {
            callback(null, value.toObject());
...
module.exports.findManyPostsById = function (posts_id, options, callback) {
  ...
  if (
    posts_id &&
    Array.isArray(posts_id) &&
    posts_id.length > 0 &&
    posts_id.filter((e) => {
      return mongoose.isValidObjectId(e);
    }).length == posts_id.length
  ) {
    posts_id = posts_id.map((e) => {
      return new ObjectId(e);
    });
    Post.find({ _id: posts_id }, null, opts)
      .then((value) => {
        try {
...
module.exports.findOnePost = function (tab_field, value, options, callback) {
  ...
  if (
    ...
    Post.findOne({ $or: obj_find }, null, opts)
      .then((value) => {
        if (value) {
          callback(null, value.toObject());
        } else {
...
module.exports.findManyPosts = function (
  search,
  field,
  limit,
  page,
  ...
  if (
    ...
    Post.countDocuments(query_mongo)
      .then((count) => {
        if (count > 0) {
...
module.exports.updateOnePost = function (post_id, update, options, callback) {
  ...
  if (post_id && mongoose.isValidObjectId(post_id)) {
    Post.findByIdAndUpdate(new ObjectId(post_id), update, {
      ...
      .then((value) => {
        try {
          ...
          else callback({ msg: "Post non trouvé.", type_error: "no-found" });
        } catch (e) {
          callback(e);
        }
      })
...
module.exports.updateManyPosts = function (
  posts_id,
  update,
  options,
  callback
  ...
  if (
    ...
    Post.updateMany({ _id: posts_id }, update, { runValidators: true })
      .then((value) => {
        try {
  ...
  if (post_id && mongoose.isValidObjectId(post_id)) {
    Post.findByIdAndDelete(post_id)
      .then((value) => {
        try {
          if (value) {
            if (value.contentImage) {
              const oldImagePath = path.join(
                __dirname,
                value.contentImage.replace("http://localhost:3000", "..")
              );
              console.log("LAAAAAAAAAAAAAAAAAAAAAA", oldImagePath);
              fs.access(oldImagePath, fs.constants.F_OK, (err) => {
                if (!err) {
                  fs.unlink(oldImagePath, (err) => {
                    if (err) {
                      console.error(
                        `Échec de la suppression de l'image à ${oldImagePath}: ${err.message}`
                      );
                    } else {
                      console.log(
                        `Image supprimée avec succès à ${oldImagePath}`
                      );
                    }
                  });
                } else {
                  console.error(`Image non trouvée à ${oldImagePath}`);
                }
              });
            }
            callback(null, value.toObject());
          ...
          console.log("Erreur lors de la suppression du post : ", e);
          callback(e);
        }
      })
      .catch((e) => {
        console.log("Erreur MongoDB lors de la suppression du post : ", e);
        callback({
          msg: "Impossible de chercher l'élément.",
          type_error: "error-mongo",
        });
