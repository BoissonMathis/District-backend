...
module.exports.repost = async function (user_id, post_id, options, callback) {
  if (
    user_id &&
    ...
    try {
      ...
      if (postToLike.user.toString() == user_id) {
        return callback({
          msg: "Le créateur du post ne peux pas repost",
          type_error: "no-valid",
        });
      }

      if (postToLike) {
        if (postToLike.repost && !postToLike.repost.includes(user_id)) {
          postToLike.repost.push(user_id);
...
module.exports.cancelrepost = async function (
  user_id,
  post_id,
  options,
  callback
  ...
  if (user_id && mongoose.isValidObjectId(user_id)) {
    try {
      ...
      if (post) {
        if (post.repost && post.repost.includes(user_id)) {
