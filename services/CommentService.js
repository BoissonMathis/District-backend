...
module.exports.findManyCommentsById = function (comments_id, options, callback) {
    let opts = {populate: (options && options.populate ? ["user", "post"] : []), lean: true}
...
module.exports.findManyComments = function(search, field, limit, page, options, callback) {
    ...
    if (typeof page !== "number" || typeof limit !== "number" || isNaN(page) || isNaN(limit)) {
        ...
        if (Types.ObjectId.isValid(search)) {
            ...
            return callback({ msg: "La valeur de recherche n'est pas un ID valide.", type_error: "no-valid" });
        }
    ...
    if (comment_id && mongoose.isValidObjectId(comment_id)) {
        if (update.like && Array.isArray(update.like)) {
            ...
            if (invalid_id.length > 0) {
                callback({ msg: "Le tableau 'like' contient des ID non valides.", type_error: 'no-valid' });
            }
...
module.exports.updateManyComments = function (comments_id, update, options, callback) {
    ...
    if (comments_id && Array.isArray(comments_id) && comments_id.length > 0 && comments_id.every(mongoose.isValidObjectId)) {
        ...
        if (update.like && Array.isArray(update.like)) {
            ...
            Comment.find({ _id: { $in: comments_id } })
                .then((comments) => {
                    if (comments.length === 0) {
                        return callback({ msg: 'Commentaires non trouvés', type_error: 'no-found' });
                    }
                    ...
                    const bulkOps = comments.map((comment) => ({
                        updateOne: {
                            ...
                            update: { like: comment.like, updated_at: comment.updated_at, user: update.user }
                        }
                    }));
            ...
            Comment.updateMany({ _id: { $in: comments_id } }, update, { runValidators: true })
                .then((value) => {
                    try {
                        if (value && value.matchedCount !== 0) {
                            callback(null, value);
                        } else {
                            callback({ msg: 'Commentaires non trouvés', type_error: 'no-found' });
                        }
                    } catch (e) {
                        callback(e);
                    }
