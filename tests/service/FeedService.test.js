...
describe("updateOneUser", () => {
    it("Ajouter des follows a un utilisateur. - S", (done) => {
        UserService.updateOneUser(tab_id_users[0], { follows: [tab_id_users[1], tab_id_users[2]] }, null, function (err, value) {
...
describe("GET - /feed", () => {
    it("Récupérer le feed de l'utilisateur connecté correctement - S", (done) => {
        FeedService.getUserConnectedFeed(tab_id_users[0], {populate: true}, function(err, value){
    ...
    it("Récupérer le feed de l'utilisateur connecté correctement qui ne suit perdsonne - S", (done) => {
        FeedService.getUserConnectedFeed(tab_id_users[1], {populate: true}, function(err, value){
            expect(value).to.haveOwnProperty('posts')
    ...
    it("Récupérer le feed d'un utilisateur inexistant - E", (done) => {
        FeedService.getUserConnectedFeed("66a748cf330b803fb1cfaac3", {populate: true}, function(err, value){
    ...
    it("Récupérer le feed d'un utilisateur avec id incorrect - E", (done) => {
        FeedService.getUserConnectedFeed("741852963", {populate: true}, function(err, value){
            expect(err).to.haveOwnProperty('msg')
            expect(err).to.haveOwnProperty('type_error')
