...
describe("POST - /login", () => {
    ...
    it("Authentifier un utilisateur incorrect. (sans username) - E", (done) => {
        chai.request(server).post('/login').send({username: '', password: 'lut'})
        .end((err, res) => {
...
describe("GET - /user", () => {
    it("Chercher un utilisateur par un champ selectionné. - S", (done) => {
        chai.request(server).get('/user').auth(valid_token, { type: 'bearer' }).query({fields: ['username'], value: 'Lutfu'})
        .end((err, res) => {
            res.should.have.status(200)
            done()
        })
    ...
    it("Chercher un utilisateur avec un champ non autorisé. - E", (done) => {
        chai.request(server).get('/user').auth(valid_token, { type: 'bearer' }).query({fields: ['bio'], value: users[0].bio})
        .end((err, res) => {
            res.should.have.status(405)
            done()
        })
    ...
    it("Chercher un utilisateur sans tableau de champ. - E", (done) => {
        chai.request(server).get('/user').auth(valid_token, { type: 'bearer' }).query({value: users[0].username})
        .end((err, res) => {
            res.should.have.status(405)
            done()
        })
...
describe("PUT - /user", () => {
    ...
    it("Modifier un utilisateur avec un id invalide. - E", (done) => {
        chai.request(server).put('/user/123456789').auth(valid_token, { type: 'bearer' }).send({username: "Olivier", bio: "Edouard"})
        .end((err, res) => {
            res.should.have.status(405)
            done()
        })
    ...
    it("Modifier un utilisateur avec un id inexistant. - E", (done) => {
        chai.request(server).put('/user/66791a552b38d88d8c6e9ee7').auth(valid_token, { type: 'bearer' }).send({username: "Olivier", bio: "Edouard"})
        .end((err, res) => {
            res.should.have.status(404)
            done()
        })
    ...
    it("Modifier un utilisateur avec un champ requis vide. - E", (done) => {
        chai.request(server).put('/user/' + users[0]._id).auth(valid_token, { type: 'bearer' }).send({ username: '', bio: "Edouard" })
        .end((err, res) => {
            res.should.have.status(405)
            done()
        })
    ...
    it("Modifier un utilisateur sans etre authentifié. - E", (done) => {
        chai.request(server).put('/user/' + users[0]._id).send({ username: "Olivier" })
        .end((err, res) => {
            res.should.have.status(401)
            done()
        })
...
describe("PUT - /users", () => {
    it("Modifier plusieurs utilisateurs. - S", (done) => {
        chai.request(server).put('/users').auth(valid_token, { type: 'bearer' }).query({id: _.map(users, '_id')}).send({ bio: "Ceci est une bio test" })
        .end((err, res) => {
            res.should.have.status(200)
            done()
        })
    ...
    it("Modifier plusieurs utilisateurs avec des ids invalide. - E", (done) => {
        chai.request(server).put('/users').auth(valid_token, { type: 'bearer' }).query({id: ['267428142', '41452828']}).send({username: "Olivier"})
        .end((err, res) => {
            res.should.have.status(405)
            done()
        })
    ...
    it("Modifier des utilisateurs avec un champ requis vide. - E", (done) => {
        chai.request(server).put('/users').auth(valid_token, { type: 'bearer' }).query({id: _.map(users, '_id')}).send({ username: ""})
        .end((err, res) => {
            res.should.have.status(405)
            done()
        })
    ...
    it("Modifier plusieurs utilisateurs sans etre authentifié. - E", (done) => {
        chai.request(server).put('/users').query({id: _.map(users, '_id')}).send({ username: "lucas" })
        .end((err, res) => {
            res.should.have.status(401)
            done()
        })
