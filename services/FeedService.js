...
module.exports.getUserConnectedFeed = async function (user_id, opts, callback) {
    ...
    try {
        ...
        const posts = await Post.aggregate([
            {
                $match: {
                    user: { $in: user.follows.map(id => new mongoose.Types.ObjectId(id)) }  // Filtrer par userId
                }
            },
            {
            ...
            {
                $sort: { createdAt: -1 }  // Trier à nouveau pour obtenir un tri global par date
            },
            {
        ...
        callback({ msg: "Erreur lors de la récupération du feed.", type_error: 'no-valid' });
    }
};